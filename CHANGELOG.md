...
- Fixed Yarn-based plugin build/install stability by enforcing a supported `glob` resolution and aligning ESLint peer dependency requirements, reducing dependency warnings and preventing lint-toolchain setup failures in CI and local environments.

###### Other changes:

- Changed `package.json`: added explicit ESLint toolchain devDependencies (`eslint`, `@types/node`, `@typescript-eslint/*`, and Salesforce ESLint configs/plugins) to keep lint configuration consistently installable.
- Changed `package.json`: added Yarn `resolutions` (`**/glob: ^13.0.3`) to force supported transitive `glob` versions.
- Updated `yarn.lock`: regenerated lock graph to apply dependency and resolution changes.

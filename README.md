# ![SFDMU](src/images/logo.png)SFDMU - the advanced Salesforce Data Loader SFDX Plugin
...
| **ScriptObject**.excludedFields      | Array<String>              | Optional                 | Array of field names that should be excluded from the original SOQL query.<br />Allows to define an array of certain fields to be excluded when using **multiselect keywords** in the object query (see the details below) |
...
| excludeIdsFromCSVFiles               | Boolean                    | Optional, Default false  | If true all record Id and lookup id columns will be omitted when producing the target CSV files. This will make CSV files created by the Plugin natively usable with any version control system. <br />The relationship between objects will still maintained using external  Id lookup columns (like Account__r.Name) |
...
* **New in version 2.8.3** - Added excludeIdsFromCSVFiles property.
